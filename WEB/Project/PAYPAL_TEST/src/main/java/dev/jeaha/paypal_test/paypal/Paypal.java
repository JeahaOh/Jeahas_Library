package dev.jeaha.paypal_test.paypal;
...
public class Paypal {
  ...
  @RequestMapping("/paypal/index")
  public String index(HttpServletRequest request, HttpServletResponse response) throws Exception {
    System.out.println("/paypal/index");
    return "/paypal/index";
  }
  package com.gmtc.paypal;
  ...
  import com.gmtc.inipay.PayManage;
  ...
  public class PaypalTestController {
    ...
    private static String PARAM_AT_VALUE = EgovProperties.getProperty("Globals.paypal.tkn.test");
    private static String URL_PAYPAL_VALIDATE = EgovProperties.getProperty("Globals.paypal.url.test");
    ...
    public String pdt_return(HttpServletRequest request, ModelMap model, HttpSession session) throws Exception {
      System.out.println("\n\tTEST_PalController.pdt_return() TEST_START :: " + new Date());
      ...
      System.out.println("\n\tTEST_PalController.pdt_return() TEST_END :: " + new Date());
      return returnUrl;
    }
    ...
    public void ipn_return(HttpServletRequest request, HttpSession session) throws Exception {
      System.out.println("\n\tTEST_PalController.ipn_return() TEST_START :: " + new Date());
      palCmnct(request, FLAG_IPN);
      System.out.println("\n\tTEST_PalController.ipn_return() TEST_END :: " + new Date());
    }
    ...
    public @ResponseBody String cncl_return(HttpServletRequest request, HttpSession session) throws Exception {
      System.out.println("\n\tTEST_PalController.cncl_return() TEST_START :: " + new Date());
      palCmnct(request, FLAG_CNCL);
      System.out.println("\n\tTEST_PalController.cncl_return() TEST_END :: " + new Date());
      return "";
    }
    ...
    private Map<String, Object> palCmnct(HttpServletRequest request, String methodFlag) throws Exception {
      //  LOG 저장을 위한 String
      LS ls = new LS();
      ls.log("\n\tTEST_PalController.palCmnt() TEST_Called From : " + methodFlag);
      ...
      if(methodFlag.equals(FLAG_CNCL)){
        ls.log("\\n\n!!!! IMPORTANT :: PAYPAL TEST_CANCEL TEST_REQUEST IS COME !!!!\n\n");
      } else {
        ls.log(":::::::::: PAYPAL TEST_REQUEST IS COME TO " + methodFlag + " AT TEST_" + new Date() + " ::::::::::\n");
      }
      
      // -> 유효성 검사를 하기 위해 파라미터(str)를 구성한다.
      Enumeration en = request.getParameterNames();
      String str = PARAM_CMD + "=" + PARAM_CMD_VALUE;
      ls.log("\n "+ methodFlag + " :: TEST_REQUEST PARAM LIST ->\n");
      ls.log(PARAM_CMD + " : " + PARAM_CMD_VALUE);
      ...
      ls.log("\n<- TEST_REQUEST PARAM LIST ::  " + methodFlag  + "\n\n");
      // <- 유효성 검사를 위한 str
      ...
      ls.log("TEST_Customer NO : " + master_no + " is duplication " + alreadyPaid);
      
      if(alreadyPaid != null && alreadyPaid == true){
        System.out.println("\tis already TEST_PAID");
        ...
        //  LOG를 파일로 저장하자.
        ls.log("\n:::::::::: PAYPAL TEST_REQUEST PROCESSIN END AT " + new Date() + " ::::::::::\n " + methodFlag + "\n");
        ls.save(master_no);
        return resultMap;
      } else {
      ...
      ls.log("\n\n" +  methodFlag + "\n:: TEST_VALIDITI TEST 1 ::\n");
      URL url = new URL(URL_PAYPAL_VALIDATE);
      ls.log(URL_PAYPAL_VALIDATE);
      ...
      try{
        ...
        ls.log("\n\t:::::TEST_ERROR:::::" + new Date());
        ls.log("\tTEST_CAUSE -> " + e.getCause());
        ls.log("\tTEST_MESSAGE -> " + e.getMessage());
        e.printStackTrace();
      } finally {
        try{ if(pw != null) { pw.close(); } } catch(Exception e) { ls.log("\tTEST_ERR MESSAGE -> " + e.getMessage()); }
      }
      // <- 유효성 검사.
      ls.log("\n" + methodFlag + "\n::TEST_VALIDITY TEST 1 URL::\n\t" + url.toString() + "\t" + str);
      ls.log("\n" + methodFlag + " \tURL :\n" + url.toString() + str);
      //  -> 유효성 검사 결과.
      String res = "";
      ...
      try {
        res = in.readLine();
        ls.log("\n" + methodFlag + "\n:: TEST_VALIDITY TEST 1 RETURN RES : \n" + res + "\n");
      } catch(Exception e) {
        ls.log("\n\t:::::TEST_ERROR:::::" + new Date());
        ls.log("\tTEST_CAUSE -> " + e.getCause());
        ls.log("\tTEST_MESSAGE -> " + e.getMessage());
      ...
      if (res.equals(RESPONSE_SUCCESS)) {
        ...
        ls.log("\n" + methodFlag + "\n::: TEST_PAYPAL VALIDITY TEST :::\nSTATUS : " + res);
      } else {
        ls.log("\n" + methodFlag + "\n!!! TEST_PAYPAL VALIDITY TEST !!!\nSTATUS : " + res+ "\nSERVER RESPONSE VALUE IS NULL\n\n");
      }
      ls.log("\n" + methodFlag + "\n\n:::TEST_PAYPAL VALIDITY TEST 2 END:::\n\n");
      //  <- 결제 유효성
      
      ls.log("\n" + methodFlag + ":: PAYMENT TEST_VALIDITY TEST END SAVING START ::");

      //  log를 파일에 저장할 때 사용할 변수   결제자 이름, 결제 완료 시간
      String customerMasterNo = "TEST_Customer Master No : ";
      
      //  -> 결제 데이터가 없고 결제 정보가 유효하다면 객체로 받자. 다만 예외가 잘 나니 예외처리 하자.
      ls.log("\n" + methodFlag + "\n:: SAVE TEST_RESULT TO DB ::");
      try{
        ls.log(isSuccessed.toString());
        PayManage payManage = new PayManage();
        if(isSuccessed){
          ...
          sb.append("<p>TEST_Result Faild</p>");
          sb.append("<p>TEST_Result Code : " + resultCode +"</p>");
          resultMap.put("resultMsg", sb );
        }
      } catch(Exception e) {
        ls.log("\n\t:::::TEST_ERROR:::::" + new Date());
      ...
      if(methodFlag.equals(FLAG_CNCL)){
        ls.log("\n" + methodFlag + "\n\n!!!!!!!!!!!!!!! IMPORTANT :: PAYPAL TEST_CANCEL REQUEST IS END !!!!!!!!!!!!!!!\n\n");
      }
      
      ls.log("\n" + methodFlag + "\n:::::::::: PAYPAL TEST_REQUEST PROCESSIN END AT " + new Date() + " ::::::::::\n");
      //  LOG를 파일로 저장하자.
      ls.log("\n\tTEST_PalController.palCmnt() TEST_END!!");
      ls.save(customerMasterNo);
